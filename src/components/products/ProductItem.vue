...
<template>
  <v-card height="360" width="250" min-width="250" elevation="2" class="d-flex flex-column align-content-space-evenly mx-auto">
    <v-img :src="productImage" @error="onImgError" :alt="props.product.name" width="250" min-height="167" max-height="166"/>
    ...
    <div class="product-item-minimum-amount-container">
      <v-card-text v-if="isMinimumAmount" class="py-0 text-orange-accent-2">Minimum quantity to buy: {{ props.product.minOrderAmount }}</v-card-text>
    </div>
    <div class="product-item-error-container mt-2">
      <v-card-text v-if="errorMessage" class="py-0 text-red-accent-3">{{ errorMessage }}</v-card-text>
    </div>
    <v-card-actions class="py-0">
      <v-card-text class="font-weight-bold text-h6 px-2 mt-1">${{ props.product.price }}</v-card-text>
      <v-btn :disabled="isAddToCartDisabled" variant="tonal" color="blue" @click="addToCart">{{ buttonText }}</v-btn>
    </v-card-actions>
  </v-card>
</template>
